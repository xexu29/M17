<?php
    ...
    if( isset($_POST['transfer_amount']) && isset($_POST['receiver']) ){
        ...
        if( $_POST['transfer_amount'] > 0 ){

            if( $selectUser_Info['money'] >= $_POST['transfer_amount'] ){

                $sender_new_money = $selectUser_Info['money'] - $_POST['transfer_amount'];
                ...
                $selectReceiver  -> execute(array('authority' => $_POST['receiver']));
                $selectReceiver_Info = $selectReceiver  -> fetch();
    
                $receiver_new_money = $selectReceiver_Info['money'] + $_POST['transfer_amount'];
            ...
            <div class="row pt-2">
                ...
                <div class="col-md-6">
                    ...
                    if( isset($_POST['status']) ){
                        ...
                        if($_POST['status'] == "unsuccess"){
                            echo '<div class="alert alert-danger mt-2" role="alert">'
                            .$strings['alert_unsuccess'].
                            '</div>';
                        }
                        if($_POST['status'] == "no_money"){
                            echo '<div class="alert alert-danger mt-2" role="alert">'
    ...
    function Post() {
        $.ajax({
            ...
            success: function(incoming) { 
                ...
                document.POSTElementById("form").reset();
